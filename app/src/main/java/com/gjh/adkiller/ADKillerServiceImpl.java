...
public class ADKillerServiceImpl {

    private static final String TAG = "ADKillerServiceImpl";
    private AccessibilityService service;

    private Settings mSetting;

    // broadcast receiver handler广播接收机处理器
    private ADKillerServiceReceiver installReceiver;
    ...
    public ADKillerServiceImpl(AccessibilityService service) {
        this.service = service;
    }

    public void onServiceConnected() {
        try {
            // the following codes are not necessary以下代码不是必需的
//            // set accessibility configuration设置辅助功能配置
//            AccessibilityServiceInfo asi = service.getServiceInfo();
//
//            // If you only want this service to work with specific applications, set their
//            // package names here. Otherwise, when the service is activated, it will listen
//            // to events from all applications.
//              如果您只希望此服务与特定应用程序一起使用，请在此处设置它们的程序包名称。否则，当服务被激活时，它将侦听来自所有应用程序的事件。
//            // Set the type of feedback your service will provide.设置您的服务将提供的反馈类型。
//            asi.feedbackType = AccessibilityServiceInfo.FEEDBACK_ALL_MASK;
//
//            // Default services are invoked only if no package-specific ones are present仅当不存在特定于包的服务时，才会调用默认服务
//            // for the type of AccessibilityEvent generated. This service *is*用于生成的AccessibilityEvent的类型。这项服务是*
////            asi.packageNames = new String[] {"com.example.android.myFirstApp", "com.example.android.mySecondApp"};
//
//            // application-specific, so the flag isn't necessary. If this was a
//            // general-purpose service, it would be worth considering setting the
//            // DEFAULT flag.特定于应用程序，因此不需要该标志。如果这是一个通用服务，那么应该考虑设置默认标志。
...
//            Log.d(TAG, keyWordList.toString());
            ...
            // whitelist of packages 包裹清单
            setWhiteList = mSetting.getWhitelistPackages();

            // load pre-defined widgets or positions 加载预定义的小部件或位置
            mapPackageWidgets = mSetting.getPackageWidgets();
            mapPackagePositions = mSetting.getPackagePositions();

            // collect all installed packages 收集所有已安装的软件包
            packageManager = service.getPackageManager();
            updatePackage();

            // install receiver and handler for broadcasting events 安装广播事件的接收器和处理程序
            InstallReceiverAndHandler();

            // create future task 创建未来任务
    ...
    private void InstallReceiverAndHandler() {
        // install broadcast receiver for package add / remove 安装程序包添加/删除的广播接收器
        installReceiver = new ADKillerServiceReceiver();
        ...
        // install handler to handle broadcast messages 安装处理程序以处理广播消息
        receiverHandler = new Handler(new Handler.Callback() {
            @Override
            public boolean handleMessage(Message msg) {
                switch (msg.what) {
                    case ADKillerService.ACTION_REFRESH_KEYWORDS:
                        keyWordList = mSetting.getKeyWordList();
//                        Log.d(TAG, keyWordList.toString());
                        break;
                    case ADKillerService.ACTION_REFRESH_PACKAGE:
                        setWhiteList = mSetting.getWhitelistPackages();
//                        Log.d(TAG, pkgWhiteList.toString());
                        updatePackage();
                        break;
                    case ADKillerService.ACTION_REFRESH_CUSTOMIZED_ACTIVITY:
                        mapPackageWidgets = mSetting.getPackageWidgets();
                        mapPackagePositions = mSetting.getPackagePositions();
//                        Log.d(TAG, mapActivityWidgets.keySet().toString());
//                        Log.d(TAG, mapActivityPositions.keySet().toString());
                        break;
                    case ADKillerService.ACTION_STOP_SERVICE:
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                            service.disableSelf();
                        }
                        break;
                    case ADKillerService.ACTION_ACTIVITY_CUSTOMIZATION:
    ...
    // events sequence after clicking one the app icon on home 单击主页上的一个应用程序图标后的事件序列
...
//    TYPE_WINDOW_CONTENT_CHANGED - com.android.systemui - android.widget.FrameLayout
    ...
    // 思路描述：
    // Window state changed - represents the event of opening a PopupWindow, Menu, Dialog, etc. 窗口状态已更改-表示打开弹出窗口、菜单、对话框等的事件。
    // Window content changed - represents the event of change in the content of a window. This change can be adding/removing view, changing a view size, etc. 窗口内容已更改-表示窗口内容的更改事件。此更改可以是添加/删除视图、更改视图大小等。
            ...
            switch (event.getEventType()) {
                case AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED:
                    if(setIMEApps.contains(tempPkgName)) {
                        // IME might be temporarily started in the package, skip this event IME可能在包中临时启动，请跳过此事件
                        break;
                    }
                    ...
                    if(!currentPackageName.equals(pkgName)) {
                        // new package, is it a activity? 新套餐，是活动吗？
                        if(isActivity) {
                            // yes, it's an activity 是的，这是一项活动
                            // since it's an activity in another package, it must be a new activity, save them 因为它是另一个包中的活动，所以它必须是新活动，请保存它们
                            currentPackageName = pkgName;
                            currentActivityName = actName;

                            // stop current skip ad process if it exists 停止当前跳过ad进程（如果存在）
                            stopSkipAdProcess();

                            if(setPackages.contains(pkgName)) {
                                // if the package is in our list, start skip ads process 如果包在我们的列表中，请启动跳过广告过程
                                // this is the only place to start skip ad process 这是开始跳过广告过程的唯一地方
                                startSkipAdProcess();
                            }
                        }
                    } else {
                        // current package, we just save the activity 当前包，我们只保存活动
                        if(isActivity) {
                            // yes, it's an activity 是的，这是一项活动
                            if(!currentActivityName.equals(actName)) {
                                // new activity in the package, this means this activity is not the first activity any more, stop skip ad process 包中的新活动，这意味着此活动不再是第一个活动，请停止跳过ad进程
                                // update: there are some cases that ad-activity is not the first activity in the package, so don't stop skip ad process 更新：有些情况下，广告活动不是包中的第一个活动，所以不要停止跳过广告过程
//                                stopSkipAdProcess();
                                currentActivityName = actName;
                                break;
                            }
                    ...
                    // now to take different methods to skip ads 现在采取不同的方法跳过广告
...
//                        Log.d(TAG, "method by position in STATE_CHANGED");
                        ...
                        if (packagePositionDescription != null) {
                            // the following codes might be run multiple times 以下代码可能会运行多次
                                ...
                                public void run() {
                                    if (num < packagePositionDescription.number) {
                                        if(currentActivityName.equals(packagePositionDescription.activityName)) {
                                            // current activity is null, or current activity is the target activity 当前活动为空，或者当前活动是目标活动
//                                            Log.d(TAG, "Find skip-ad by position, simulate click now! ");
                            ...
                            // no customized positions for this package, don't try this method again 此包没有自定义位置，请不要重试此方法
...
//                            Log.d(TAG, "Find skip-ad by widget, simulate click ");
                            ...
                            // no customized widget for this package, don't try this method again 没有此包的自定义小部件，请不要重试此方法
                            b_method_by_activity_widget = false;
                        }
                    }

                    if (b_method_by_button_keyword) {
//                        Log.d(TAG, "method by keywords in STATE_CHANGED"); 状态改变
...
//        showAllChildren(root);
        ...
        while (index < total && !isFind) {
            ...
            if (node != null) {
                ...
                // try to find keyword 尝试查找关键字
                for (String keyword: keyWordList) {
                    // text or description contains keyword, but not too long （<= length + 6）文本或说明包含关键字，但不太长（<=长度+6）
...
//                        Log.d(TAG, "self clicked = " + clicked);
            ...
            // reach the end of listA 到达listA的尽头
            if (index == total) {
                listA = listB;
                listB = new ArrayList<>();
                index = 0;
    ...
    /**
     * 查找并点击由 ActivityWidgetDescription 定义的控件   活动小部件描述
     */
...
//            Log.d(TAG, indent + Utilities.describeAccessibilityNode(e));
    ...
    /**
     * start the skip-ad process 启动跳过广告过程
     */
...
//        Log.d(TAG, "Start Skip-ad process");
        ...
        // cancel all methods 4 seconds later 4秒后取消所有方法
    ...
    /**
     * stop the skip-ad process 停止跳过广告过程
     */
...
//        Log.d(TAG, "Stop Skip-ad process");
    ...
    /**
     * stop the skip-ad process, without cancel scheduled task 停止跳过ad进程，不取消计划任务
     */
    ...
    /**
     * find all packages while launched. also triggered when receive package add / remove events 在启动时查找所有包。接收包添加/删除事件时也会触发
     */
...
//        Log.d(TAG, "updatePackage");
        ...
        // find all input methods 查找所有输入法
        ...
        // ignore some packages in hardcoded way 以硬编码方式忽略某些包
        // https://support.google.com/a/answer/7292363?hl=en
        setTemps.add(this.packageName);
        setTemps.add("com.android.settings");

        // remove whitelist, systems, homes & ad-hoc packages from pkgLaunchers 从pkgLaunchers中删除白名单、系统、家庭和特别软件包
...
//        Log.d(TAG, "Working List = " + pkgLaunchers.toString());
    ...
    // display activity customization dialog, and allow users to pick widget or positions 显示“活动自定义”对话框，并允许用户选择小部件或位置
    @SuppressLint("ClickableViewAccessibility")
    private void showActivityCustomizationDialog() {
        // show activity customization window 显示活动自定义窗口
        ...
        // activity customization view 活动自定义视图
    ...
    public void ShowToastInIntentService(final String sText) {
        ...
        // show one toast in 5 seconds only 仅在5秒钟内展示一片吐司
